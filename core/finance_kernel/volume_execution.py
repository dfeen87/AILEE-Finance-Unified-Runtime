...
class VolumeExecutionOperator(BaseOperator):
    ...
    def process_tick(self, advisory_data: Dict[str, Any], current_price: float, safety_state: Optional[Dict[str, Any]] = None):
        ...
        if self.consecutive_bars < self.hysteresis_bars:
            self.log_audit("DEBOUNCE_WAIT", f"Signal pending confirmation: {signal_type}", advisory_data, current_price)
            return

        # 4. Position alignment check
        if desired_side == self.current_position_side and desired_side != "FLAT":
            self.log_audit("HOLD", f"Position already aligned with signal: {signal_type}", advisory_data, current_price)
            return

        if not self.enable_auto_execute:
            self.log_audit("DRY_RUN_SIGNAL", f"Auto-execution disabled. Would execute: {signal_type}", advisory_data, current_price)
            return

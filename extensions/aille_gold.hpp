...
#ifndef AILLE_GOLD_HPP
#define AILLE_GOLD_HPP
...
struct alignas(64) GOLDState {
    ...
    constexpr GOLDState() : price(0.0f), realized_vol(0.0f), drawdown(0.0f), trend_score(0.0f), smoothed_vol(0.0f), _reserved_padding{} {}
};
static_assert(sizeof(GOLDState) == 64, "GOLDState must be exactly 64 bytes");

struct alignas(64) GOLDAdvisory {
    ...
    constexpr GOLDAdvisory() : recommended_weight(1.0f), risk_score(0.0f), risk_elevated(false), growth_favorable(true), _reserved_padding{} {}
};
static_assert(sizeof(GOLDAdvisory) == 64, "GOLDAdvisory must be exactly 64 bytes");

struct alignas(64) GOLDObservabilityMetrics {
    float risk_score;
    ...
    constexpr GOLDObservabilityMetrics() : risk_score(0.0f), volatility_band(0.0f), advisory_weight(0.0f), trend_score(0.0f), _reserved_padding{} {}
};
static_assert(sizeof(GOLDObservabilityMetrics) == 64, "GOLDObservabilityMetrics must be exactly 64 bytes");

[[nodiscard]] constexpr GOLDAdvisory evaluate_gold_state(
